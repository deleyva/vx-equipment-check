#!/usr/bin/env python3
"""
Aplicación de verificación de equipos de escritorio - Versión ttkbootstrap sin PIL
...
class VitalinuxEquipmentCheckApp:
    ...
    def create_widgets(self):
        ...
        scrollbar = tk.Scrollbar(self.root, orient="vertical", command=canvas.yview, bg='#e9ecef')
        ...
        # Configurar scroll simple
        self.bind_mousewheel(canvas)
        
    def bind_mousewheel(self, canvas):
        """Scroll suave por píxeles - sin saltos"""
        def _on_mousewheel(event):
            # Scroll suave por píxeles - compatible con todos los sistemas
            try:
                if hasattr(event, 'delta') and event.delta:
                    # macOS y Windows - scroll por píxeles
                    delta = event.delta
                    # Obtener la posición actual del scroll
                    top, bottom = canvas.yview()
                    
                    # Calcular el movimiento en píxeles (muy suave)
                    if abs(delta) < 120:
                        # Trackpad - movimiento muy suave
                        scroll_amount = delta * 0.001
                    else:
                        # Rueda del ratón - movimiento suave
                        scroll_amount = delta * 0.0005
                    ...
                    # Linux - scroll suave por píxeles
                    top, bottom = canvas.yview()
                    if event.num == 4:
                        # Scroll hacia arriba
                        new_top = max(0, top - 0.02)
                        canvas.yview_moveto(new_top)
                    elif event.num == 5:
                        # Scroll hacia abajo
                        new_top = min(1, top + 0.02)
                        canvas.yview_moveto(new_top)
            except:
                # Fallback si hay algún error
                pass
        
        # Bind directo al canvas y a la ventana
    ...
    def create_component_section(self, parent, title, estado_var, component_type):
        # Frame para cada componente - sin bootstyle para evitar PIL
        ...
        # Crear radiobuttons personalizados usando botones normales
        correcto_frame = tk.Frame(radio_frame, bg='#ffffff')
        correcto_frame.pack(side=LEFT, padx=(0, 60))
        
        # Círculo personalizado para "Correcto"
        correcto_circle = tk.Label(
            correcto_frame,
            text="○",  # Círculo vacío
            font=('Arial', 20),
            fg='#cccccc',  # Gris claro inicial
            bg='#ffffff',
            cursor='hand2'
        )
        correcto_circle.pack(side=LEFT, padx=(0, 10))
        
        # Texto para "Correcto"
        correcto_label = tk.Label(
            correcto_frame,
        ...
        correcto_label.pack(side=LEFT)
        
        # Crear radiobuttons personalizados para "Defectuoso"
        defectuoso_frame = tk.Frame(radio_frame, bg='#ffffff')
        defectuoso_frame.pack(side=LEFT)
        
        # Círculo personalizado para "Defectuoso"
        defectuoso_circle = tk.Label(
            defectuoso_frame,
            text="○",  # Círculo vacío
            font=('Arial', 20),
            fg='#cccccc',  # Gris claro inicial
        ...
        defectuoso_circle.pack(side=LEFT, padx=(0, 10))
        
        # Texto para "Defectuoso"
        defectuoso_label = tk.Label(
            defectuoso_frame,
        ...
        defectuoso_label.pack(side=LEFT)
        
        # Función para manejar la selección
        def select_correcto():
            estado_var.set("correcto")
            correcto_circle.config(text="●", fg='#007bff')  # Círculo lleno azul
            defectuoso_circle.config(text="○", fg='#cccccc')  # Círculo vacío gris
            self.toggle_description(component_type, False)
        
        def select_defectuoso():
            estado_var.set("defectuoso")
            defectuoso_circle.config(text="●", fg='#007bff')  # Círculo lleno azul
            correcto_circle.config(text="○", fg='#cccccc')  # Círculo vacío gris
            self.toggle_description(component_type, True)
        
        # Bind clicks a los elementos
        correcto_circle.bind("<Button-1>", lambda e: select_correcto())
        correcto_label.bind("<Button-1>", lambda e: select_correcto())
        defectuoso_circle.bind("<Button-1>", lambda e: select_defectuoso())
        defectuoso_label.bind("<Button-1>", lambda e: select_defectuoso())
